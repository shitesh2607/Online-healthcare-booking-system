...
def doctor_dashboard(request):
	...
	context={'docapp':docapp,'cnt':cnt,'profile':profile, 'pres':pres, 'app':app, 'pre':pre}
...
def main(request):
	...
	if request.method=="POST":
		...
		with open('files/symptom_precaution.csv') as csv_file:
			...
			for row in csv_reader:
				if row[0]==result:
					precaution=[row[1],row[2],row[3],row[4]]
					precaution='\n'.join(precaution)
					print (precaution)
                

	context= {'text':result,'des':description,'pre':precaution}
